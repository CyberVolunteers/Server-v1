...
module.exports = class NodemailerManager {
	constructor(pool, logger, hostName) {
		...
		this.resetPasswordTextTemplate = Handlebars.compile(fs.readFileSync("./public/emails/resetPasswordEmail.txt", "utf8"));

		this.resetPasswordHTMLTemplate = Handlebars.compile(fs.readFileSync("./public/emails/resetPasswordEmail.hbs", "utf8"));
	...
	async sendPasswordResetCode(uuid, email, isVolunteer) {
		...
		try {
			...
			const templateInfo = {
				"link": this.hostName + "resetPassword?uuid=" + uuid + "&email=" + email + "&isVolunteer=" + isVolunteer,
				"firstName": rows[0].firstName,
				"lastName": rows[0].lastName
			};
