...
app.get("/getListing", getListingRateLimit, function (req, res, next) {
	...
	pool.query("SELECT charities.charityName, listings.timeForVolunteering, listings.duration, listings.placeForVolunteering, listings.targetAudience, listings.skills, listings.createdDate, listings.requirements, listings.opportunityDesc, listings.opportunityCategory, listings.opportunityTitle, listings.numOfvolunteers, listings.minHoursPerWeek, listings.maxHoursPerWeek, listings.pictureName, listings.scrapedCharityName, listings.latitude, listings.longitude FROM `listings` INNER JOIN charities ON listings.charityId=charities.id  WHERE `uuid`=?", [req.query.uuid], function (err, results) {
		if (err) return next(err);
